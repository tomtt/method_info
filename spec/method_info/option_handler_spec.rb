    ...
    describe "handle" do
      describe "processing options" do
        ...
        it "uses a value from the default profile if an option was not passed" do
          ...
          Formatter.should_receive(:build).with(anything, hash_including(:mock_option => :mock_value))
          MethodInfo::OptionHandler.handle(:object)
        end
        it "uses a value for an option if it was passed in" do
          MethodInfo::OptionHandler.stub!(:default_profile).and_return({ :mock_option => :mock_value })
          Formatter.should_receive(:build).with(anything, hash_including(:mock_option => :passed_value))
      ...
      it "builds an ancestor method structure with the object" do
        ...
        Formatter.should_receive(:build).with(mock_object, anything)
      ...
      it "passes through regular options when building an ancestor method structure" do
        Formatter.should_receive(:build).with(anything, hash_including(:private_methods => :some_value))
      ...
      it "does not pass the :format option when building an ancestor method structure" do
        Formatter.should_receive(:build).with(anything, hash_not_including(:format))
        MethodInfo::OptionHandler.handle(:foo, { :format => :string })
      end

      it "prints the formatter if the :format option is not set" do
        mock_ams = mock('formatter')
        Formatter.stub!(:build).and_return(mock_ams)
        OptionHandler.should_receive(:puts).with(mock_ams)
        MethodInfo::OptionHandler.handle(:foo)
      end

      it "returns the array representation of the formatter if the :format option is :array" do
        mock_ams = mock('formatter')
        Formatter.stub!(:build).and_return(mock_ams)
      ...
      it "returns the string representation of the formatter if the :format option is :string" do
        mock_ams = mock('formatter')
        Formatter.stub!(:build).and_return(mock_ams)
